...
import { getMyProfile } from '../services/warehouseStaffService';
...
const WarehouseStaffNavbar = () => {
  ...
  useEffect(() => {
    ...
    getMyProfile()
      .then((profile) => {
        if (isMounted) setWarehouseName(profile?.warehouseName || '');
      })
      .catch(() => {
        // Not assigned yet, or a transient error — the page bodies already
        // surface a clearer message, so just leave the badge blank here.
      });
    return () => {
      isMounted = false;
    };
  ...
  const links = [
    { name: 'Fulfillment', path: '/warehouse/dashboard' },
    { name: 'Stock', path: '/warehouse/stock' },
    { name: 'Returns & QC', path: '/warehouse/returns' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="bg-blue-100 text-blue-800 text-2xs font-bold px-2 py-0.5 rounded-md uppercase tracking-wide border border-blue-200/80">
              {warehouseName ? `Warehouse · ${warehouseName}` : 'Warehouse Staff'}
            </span>
          </div>
        </button>
