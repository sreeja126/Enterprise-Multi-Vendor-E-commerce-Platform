...
public class WishlistService {
    ...
    public List<WishlistItemResponseDTO> addItem(String email, Long productId) {
        // Idempotent — adding something already in the wishlist just
        // returns the current list instead of erroring, since from the
        // customer's point of view "save this" already succeeded once.
    ...
    @Transactional
    public List<WishlistItemResponseDTO> removeItem(String email, Long productId) {
        WishlistItem item = wishlistRepository.findByUserEmailAndProductId(email, productId)
                .orElseThrow(() -> new SecurityException("This item isn't in your wishlist."));
